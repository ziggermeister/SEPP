...
"""
import argparse, json, hashlib, datetime as dt, math
...
def fetch_prices(symbols, start, end):
    raw = yf.download(" ".join(symbols), start=start, end=end, auto_adjust=False,
...
def main():
    ...
    pack["stable_hash"] = stable_hash({"universe": args.symbols, "as_of": end, "mu": mu, "sigma": sig, "rho": rho_df.to_dict(), "yield_rate": yld.to_dict()})
