...
def parse_args():
    ...
    p.add_argument("--sources", default="yahoo",
                   help="Comma-separated sources: yahoo,alpha,stooq")
    p.add_argument("--consensus", default="prefer-yahoo-fill",
                   choices=["prefer-yahoo-fill","median"],
                   help="How to combine sources")
    p.add_argument("--alpha_key", default=None,
    ...
    p.add_argument("--assets_config", default="config/assets.json",
...
def run():
    ...
    try:
        from run_portfolios import (
            load_assets_config, build_category_map, cma_for_symbol
        )
        ...
        if not getattr(args, "no_cma", False):
            ...
            MU_CMA = np.array(
                [cma_for_symbol(s, m, y, catmap, anchors) for s, m, y in zip(symbols, MU_TRAIL, yld)],
                dtype=float
            )
    ...
    safe_idx = []
    try:
        from run_portfolios import load_assets_config, build_category_map
        assets_cfg = load_assets_config(getattr(args, "assets_config", "config/assets.json"))
        ...
        if getattr(args, "safe_tickers", None):
            ...
            if not safe_idx:
                print("[warn] safe_tickers override provided no matches; falling back to config categories.")
