    ...
    for _ in range(n_sims):
        ...
        try:
            ...
            L = np.linalg.cholesky(
                higham_psd(rho_i) + 1e-10 * np.eye(n_assets))
        chol_list.append(L)
...
def years_covered_forward(safe_balance,
                          yield_rate,
...
def simulate_paths(initial_value,
    ...
    safe_totals[:, 0] = np.where(np.isfinite(safe_prev_total), safe_prev_total,
                                 0.0)
    ...
    shocks = gaussian_copula_t_draws(n_sims,
                                     years,
                                     mu,
                                     sig,
                                     rho,
    ...
    for t in range(1, years + 1):
        ...
        after_price = np.where(np.isfinite(prev * (1.0 + price_r)),
                               prev * (1.0 + price_r), 0.0)
        income = np.where(np.isfinite(prev * yld), prev * yld, 0.0)
        values[:, t, :] = after_price + income

        income_total = np.where(np.isfinite(income.sum(axis=1)),
                                income.sum(axis=1), 0.0)
        principal_to_withdraw = np.maximum(0.0,
                                           annual_withdrawal - income_total)

        # cascade
        for aidx in cascade_idx:
            bal = np.where(np.isfinite(values[:, t, aidx]), values[:, t, aidx],
                           0.0)
        ...
        safe_sum = np.where(np.isfinite(np.sum(safe_now, axis=1)),
                            np.sum(safe_now, axis=1), 0.0)
...
# --------------------------- Liquidity helpers ----------------------------------
def path_liquidity_stat(safe_path,
                        yld_path,
                        draw,
                        method=LIQ_METHOD,
                        cap=LIQ_CAP):
    ...
    for t in range(1, T):
        liq_series[t - 1] = years_covered_forward(safe_path[t],
                                                  yld_path[t],
                                                  draw=draw,
                                                  cap=cap)
    if method == "min_all":
...
def liquidity_distribution_stats(safe_totals,
    ...
    for i in range(n_sims):
        liq_min = np.inf
        for t in range(1, T):
            liq_t = years_covered_forward(safe_totals[i, t],
                                          safe_yld_eff[i, t],
                                          draw=annual_withdrawal,
                                          cap=cap)
            if liq_t < liq_min:
    ...
    print(f"  Liquidity (method={method}) summary: "
...
def compute_metrics(total_values,
    ...
    for i in range(n_sims):
        liq_per_path[i] = path_liquidity_stat(safe_totals[i],
                                              safe_yld_eff[i],
                                              draw=annual_withdrawal,
                                              method=LIQ_METHOD,
                                              cap=LIQ_CAP)
    ...
    sortino = float((np.mean(ann_log_ret) - MAR) /
                    downside_dev) if downside_dev > 0 else 2.0
...
def norm_ruin(r):
    r = min(max(r, 0.0), 0.20)
    return 100.0 * (1.0 - r / 0.20)


def norm_liquidity(years):
    y = max(0.0, years)
    return 90.0 * (y / 3.0) if y <= 3.0 else 90.0 + 10.0 * min(
        (y - 3.0) / 2.0, 1.0)
...
def norm_median(ret):
    return norm_linear_floor_ceiling(ret, 0.03, 0.10)


def norm_upside(ret):
    return 100.0 * min(np.log(max(ret, 0.06) / 0.06) / np.log(20.0 / 6.0), 1.0)


def norm_cvar(es):
    return 100.0 * min((0.50 + es) / 0.40, 1.0)


def norm_sortino(s):
    return 100.0 * min(max(s, 0.0) / 2.0, 1.0)


def norm_mdd(mdd):
    return 100.0 * min((0.50 + mdd) / 0.40, 1.0)


def norm_calmar(c):
    return 100.0 * min(max(c, 0.0) / 2.0, 1.0)


def norm_egsp(p):
    return 100.0 * (1.0 - min(max(p, 0.0) / 0.50, 1.0))
...
def _precompute_path_stats(total_values,
    ...
    for i in range(n_sims):
        liq_i[i] = path_liquidity_stat(safe_totals[i],
                                       safe_yld_eff[i],
                                       draw=annual_withdrawal,
                                       method=LIQ_METHOD,
                                       cap=LIQ_CAP)
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ann_log_ret_i = np.log(
            total_values[:, -1] / total_values[:, 0]) / years
    mdd_i = np.empty(n_sims, dtype=float)
    for i in range(n_sims):
        mdd_i[i] = per_path_max_drawdown(
            total_values[i, :]) if survive[i] else np.nan
    return {
        "ruin": ruin_i,
        "liq": liq_i,
...
def bootstrap_se(total_values,
    ...
    s = _precompute_path_stats(total_values, safe_totals, safe_yld_eff,
                               egsp_flags)
    ruin_i, liq_i, egsp_i, survive, ann_log_ret, mdd_i, MAR = s["ruin"], s[
        "liq"], s["egsp"], s["survive"], s["ann_log_ret"], s["mdd"], s["MAR"]
    n = len(ruin_i)
    ...
    for b in range(n_resample):
        ...
        if surv_idx.size < max(1, int(0.05 * n)):
            ...
            downside = r[r < MAR]
            downside_dev = float(
                np.std(downside)) if downside.size > 0 else 0.0
            sortino = float(
                (np.mean(r) - MAR) / downside_dev) if downside_dev > 0 else 2.0
...
def score_portfolio(name, w, assets, mu, sig, rho, yld, safe_idx, growth_idx):
    ...
    safe_eff_yield = float(
        np.sum((w[safe_idx] / max(safe_w, 1e-12)) *
               yld[safe_idx])) if safe_w > 0 else 0.0
    liq_t0 = years_covered_forward(safe_init_balance,
                                   safe_eff_yield,
                                   draw=ANNUAL_WITHDRAWAL,
                                   cap=50)
    print(
        f"  SNAPSHOT Liquidity(t=0): safe_w={safe_w:.3f}, safe_init=${safe_init_balance:,.0f}, yld_eff={safe_eff_yield:.2%}, years≈{liq_t0:.2f}"
    )
    ...
    for regime, weight in STRESS_BLEND.items():
        ...
        if regime == "Base" and DEBUG_LIQ_STATS:
            ...
            _ = liquidity_distribution_stats(base_safe,
                                             base_yld,
                                             ANNUAL_WITHDRAWAL,
                                             cap=50,
                                             method=LIQ_METHOD)
    ...
    print(f"\n=== {name} ===")
    print(
        f"Holdings: {n_hold} | Headline Score: {round(headline,1)} ±4 (SE≈{se:.2f})"
    )
    print(
        f"Period sub-scores: {{ Yrs1-4: {round(subs['Yrs1-4'],1)}, Yrs5-8: {round(subs['Yrs5-8'],1)}, Yrs9-12: {round(subs['Yrs9-12'],1)} }}"
    )
    print("Blended Metrics (Base 60%, Front 20%, Prol 20%):")
    print(
        f" Ruin={blended['Ruin']:.3f} Liquidity={blended['Liquidity']:.2f}y MedRet={blended['Median_Return']:.3f} "
...
def main():
    ...
    print(
        f"PARAM PACK: as_of={PACK['as_of']} hash={PACK['stable_hash']} source_prices={PACK['sources']['prices']}"
    )
    ...
    for name, w in PORTFOLIOS.items():
        score_portfolio(name, w, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX,
                        GROWTH_IDX)
