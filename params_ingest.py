...
ISSUER_HINTS = {
    ...
    "IEFA": (
        "iShares",
        "https://www.ishares.com/us/products/239665/ishares-core-msci-eafe-etf",
    ),
    "QQQ": (
        "Invesco",
    ...
    "SGOV": (
        "iShares",
        "https://www.ishares.com/us/products/309470/ishares-0-3-month-treasury-bond-etf",
    ),
    "SCHD": ("Schwab", "https://www.schwabassetmanagement.com/products/schd"),
    "CDC": (
        "Victory",
        "https://victorysharesliterature.vcm.com/products/victoryshares-us-eq-inc-enhanced-volatility-wtd-etf",
    ),
    "VIG": (
        "Vanguard",
        "https://investor.vanguard.com/investment-products/etfs/profile/{ticker}",
...
def fetch_prices(tickers: List[str], start: str, end: str) -> Tuple[pd.DataFrame, str]:
    for fn, label in [
        (_tiingo_prices, "Tiingo"),
        (_nasdaqdl_prices, "NasdaqDataLink"),
...
def qa_pack(pack: Dict, prev_pack: Optional[Dict] = None) -> List[str]:
    ...
    if not np.isfinite(arr_mu).all():
        issues.append("Non-finite mu found")
    if not np.isfinite(arr_sig).all():
        issues.append("Non-finite sigma found")
    if not np.isfinite(rho).all():
        issues.append("Non-finite rho entries")
    if not np.isfinite(yld).all():
        issues.append("Non-finite yields")

    # bounds
    if (arr_sig < 0).any():
        issues.append("Negative sigma")
    if not np.allclose(np.diag(rho), 1.0, atol=1e-6):
        issues.append("rho diag not 1")
    if (np.abs(rho) > 1.0 + 1e-8).any():
        issues.append("rho outside [-1,1]")
    if (yld < 0).any() or (yld > 0.15).any():
        issues.append("yield out of [0,15%]")

    # symmetry
    if not np.allclose(rho, rho.T, atol=1e-8):
        issues.append("rho not symmetric")
    ...
    if prev_pack:
        ...
        if (dmu > 0.02).any():
            issues.append("Δmu > 2% on at least one asset")
        if (dsig > 0.05).any():
            issues.append("Δsigma > 5% on at least one asset")
        if drho > 0.15:
            issues.append(f"median |Δrho| > 0.15 (={drho:.3f})")
    return issues
