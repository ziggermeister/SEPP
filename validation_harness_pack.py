...
GOLDEN = {
    "1_Sliding_Window": {
        "blended": {"Ruin":0.0030, "Liquidity":1.918, "Median_Return":0.060, "Upside":0.158, "CVaR":-0.096, "Sortino":0.92, "MDD":-0.253, "Calmar":0.25, "Early_Sale":0.000},
        "headline": 72.3, "se": 0.72
    },
    "2_Suggested_Barbell": {
        "blended": {"Ruin":0.0223, "Liquidity":0.000, "Median_Return":0.061, "Upside":0.181, "CVaR":-0.153, "Sortino":0.56, "MDD":-0.328, "Calmar":0.20, "Early_Sale":0.002},
        "headline": 56.8, "se": 0.93
    },
    "3_Current_Diversified": {
        "blended": {"Ruin":0.0021, "Liquidity":2.702, "Median_Return":0.060, "Upside":0.153, "CVaR":-0.081, "Sortino":1.02, "MDD":-0.232, "Calmar":0.28, "Early_Sale":0.000},
        "headline": 78.1, "se": 0.90
    }
}

ACCEPT = {
    "1_Sliding_Window":    {"t0_liq_years": (5.4,5.7), "liq_p10_med": (1.6,2.6), "blended_liq": (1.7,2.2), "ruin":(0.002,0.006), "score":(70.0,75.0), "se_y1_4":(0.5,1.2)},
    "2_Suggested_Barbell": {"t0_liq_years": (2.3,2.7), "liq_p10_med": (0.00,0.10),"blended_liq": (-0.1,0.1), "ruin":(0.015,0.030),"score":(55.0,58.5), "se_y1_4":(0.6,1.3)},
    "3_Current_Diversified":{"t0_liq_years": (5.4,6.0), "liq_p10_med": (2.6,3.3), "blended_liq": (2.4,3.0), "ruin":(0.001,0.005),"score":(76.5,79.5), "se_y1_4":(0.6,1.3)},
}
...
def compute_blended_for_weights(sepp, w, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX):
    ...
    for regime, weight in sepp.STRESS_BLEND.items():
        totals, safe_totals, safe_yld_eff, egsp_flags = sepp.simulate_paths(
            sepp.INITIAL_PORTFOLIO_VALUE, w, MU, SIG, RHO, YIELD_RATE, sepp.YEARS,
            sepp.ANNUAL_WITHDRAWAL, SAFE_IDX, GROWTH_IDX, assets, n_sims=sepp.N_SIM, seed=sepp.SEED, regime=regime
        )
        metrics = sepp.compute_metrics(totals, safe_totals, safe_yld_eff, egsp_flags,
                                       sepp.ANNUAL_WITHDRAWAL, sepp.MIN_ACCEPTABLE_RETURN, sepp.YEARS)
        if blended is None: blended = {k: 0.0 for k in metrics.keys()}
        for k, v in metrics.items(): blended[k] += weight * v
        if regime == "Base":
    ...
    for i in range(n_sims):
        liq_per_path[i] = sepp.path_liquidity_stat(base_safe[i], base_yld[i], draw=sepp.ANNUAL_WITHDRAWAL, method=sepp.LIQ_METHOD, cap=sepp.LIQ_CAP)
    liq_per_path = np.where(np.isfinite(liq_per_path), liq_per_path, 0.0)

    # t0 snapshot liquidity
    safe_w = float(np.sum(w[SAFE_IDX])); safe_init = sepp.INITIAL_PORTFOLIO_VALUE*safe_w
    safe_eff_yield = float(np.sum((w[SAFE_IDX] / max(safe_w, 1e-12)) * YIELD_RATE[SAFE_IDX])) if safe_w > 0 else 0.0
    t0_liq = sepp.years_covered_forward(safe_init, safe_eff_yield, draw=sepp.ANNUAL_WITHDRAWAL, cap=50)

    return blended, float(round(headline,1)), subs, float(se), liq_per_path, float(t0_liq)
...
def check_range(label, val, lo, hi, failures, pfx=""):
    ...
    if not ok: failures.append((pfx+label, val, lo, hi))
...
def golden_master_check(result_by_name):
    ...
    for name, res in result_by_name.items():
        ...
        for k, gv in G["blended"].items():
            ...
            if not ok: failures.append(f"{name}: blended[{k}] {v:.6f} != {gv:.6f}")
        # headline
...
def metamorphic_checks(sepp, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX, w_baseline, n_sims=800):
    # Run with baseline vs zero-withdraw vs doubled safe yield; check monotonicity
    def blended_for(draw, yld_override=None):
        yld = YIELD_RATE.copy()
        if yld_override is not None: yld = yld_override
        blended = None
        for regime, weight in sepp.STRESS_BLEND.items():
            totals, safe_totals, safe_yld_eff, egsp_flags = sepp.simulate_paths(
                sepp.INITIAL_PORTFOLIO_VALUE, w_baseline, MU, SIG, RHO, yld, sepp.YEARS,
                draw, SAFE_IDX, GROWTH_IDX, assets, n_sims=n_sims, seed=sepp.SEED, regime=regime
            )
            m = sepp.compute_metrics(totals, safe_totals, safe_yld_eff, egsp_flags, draw, sepp.MIN_ACCEPTABLE_RETURN, sepp.YEARS)
    ...
    totals, safe_totals, safe_yld_eff, egsp_flags = sepp.simulate_paths(
        sepp.INITIAL_PORTFOLIO_VALUE, w_baseline, MU, SIG, RHO, YIELD_RATE, sepp.YEARS,
        sepp.ANNUAL_WITHDRAWAL, SAFE_IDX, GROWTH_IDX, assets, n_sims=n_sims, seed=sepp.SEED, regime="Base"
    )
    n_s = safe_totals.shape[0]
    med_vals = np.array([sepp.path_liquidity_stat(safe_totals[i], safe_yld_eff[i], draw=sepp.ANNUAL_WITHDRAWAL, method="median_all", cap=sepp.LIQ_CAP) for i in range(n_s)])
    p10_vals = np.array([sepp.path_liquidity_stat(safe_totals[i], safe_yld_eff[i], draw=sepp.ANNUAL_WITHDRAWAL, method="p10_yr1_8", cap=sepp.LIQ_CAP) for i in range(n_s)])
    ...
    print(f"  Liqu baseline={base['Liquidity']:.2f} vs zero-draw={zero['Liquidity']:.2f} (must increase)")
    print(f"  Liqu baseline={base['Liquidity']:.2f} vs doubled-safe-yield={dy['Liquidity']:.2f} (must not decrease)")
    print(f"  median_all - p10_yr1_8 (mean diff) = {np.mean(med_vals - p10_vals):.3f} (must be >= 0)")
    print("### METAMORPHIC:", "PASS ###" if (ok1 and ok2 and ok3 and ok4) else "FAIL ###")
    return ok1 and ok2 and ok3 and ok4

def compare_bootstrap_methods(sepp, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX, portfolios):
    # Tiny N for speed
    _N_SIM = 300; _B = 250
    out = {}
    ...
    for name, w in portfolios.items():
        totals, safe_totals, safe_yld_eff, egsp_flags = sepp.simulate_paths(
            sepp.INITIAL_PORTFOLIO_VALUE, w, MU, SIG, RHO, YIELD_RATE, sepp.YEARS,
            sepp.ANNUAL_WITHDRAWAL, SAFE_IDX, GROWTH_IDX, assets, n_sims=_N_SIM, seed=sepp.SEED, regime="Base"
        )
        # fast
        se_fast = sepp.bootstrap_se(totals, safe_totals, safe_yld_eff, egsp_flags, sepp.count_holdings(w), n_resample=_B)
        # slow
        def slow_bootstrap_se():
            ...
            for _ in range(_B):
                ...
                m = sepp.compute_metrics(totals[idx], safe_totals[idx], safe_yld_eff[idx], egsp_flags[idx],
                                         sepp.ANNUAL_WITHDRAWAL, sepp.MIN_ACCEPTABLE_RETURN, sepp.YEARS)
...
def main():
    ...
    start = datetime.now(); t0 = perf_counter()
    ...
    # Print pack metadata
    print(f"PARAM PACK: as_of={PACK['as_of']} hash={PACK['stable_hash']} monthly_obs={PACK.get('monthly_obs')}")
    ...
    result_by_name = {}
    for name, w in sepp.PORTFOLIOS.items():
        blended, headline, subs, se, liq_per_path, t0_liq = compute_blended_for_weights(sepp, w, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX)
        ...
        check_range("liq_p10_med",  float(np.median(liq_per_path)), *R["liq_p10_med"],  all_failures)
    ...
    end = datetime.now(); t1 = perf_counter()
    print(f"\nEnd:   {end}")
    ...
    meta_ok  = metamorphic_checks(sepp, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX, list(sepp.PORTFOLIOS.values())[2], n_sims=800)
    boot_ok  = compare_bootstrap_methods(sepp, assets, MU, SIG, RHO, YIELD_RATE, SAFE_IDX, GROWTH_IDX, sepp.PORTFOLIOS)
